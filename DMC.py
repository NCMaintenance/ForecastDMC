            ...
            for hospital in selected_hospitals:
                ...
                for target_col_name in ['ED Beds', 'Trolleys']:
                    ...
                    model = lgb.LGBMRegressor(
                        n_estimators=min(1000, len(X_train) * 2), # Adapt n_estimators to data size
                        learning_rate=0.1,
                        max_depth=min(6, len(available_features) // 2 + 1), # Adapt max_depth
                        num_leaves=min(40, 2 ** min(6, len(available_features) // 2 + 1) - 1), # Adapt num_leaves
                        subsample=0.8,
